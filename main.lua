...
Title.TextSize = 18
Title.Text = "slaimmials Yield v1.0.0"
...
Credits.Text = "Recreate by slaimmials"
		...
		if pcall(function() readfile("IY_FE.iy") end) then
			if readfile("IY_FE.iy") ~= nil then
				local success, response = pcall(function()
					...
					if json.prefix ~= nil then prefix = json.prefix else prefix = '!sy' end
...
PrefixBox:GetPropertyChangedSignal("Text"):Connect(function()
	...
	Cmdbar.PlaceholderText = "Command... (!sy)"
	updatesaves()
end)
...
IndexContents = function(str,bool,cmdbar,Ianim)
	...
	if str:sub(1,1) == "!sy" then str = str:sub(2) end
...
function autoComplete(str,curText)
	...
	if curText:sub(subPos+1,subPos+1) == "!sy" then subPos = subPos + 1 end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'print [text] / prnt [text]', DESC = 'Returns your text to dev console'}
wait()
...
addcmd('print', {'prnt'}, function(args, speaker)
	print(args[1])
end)
